...
def calculate_demographic_data(print_data=True):
    ...
    num_min_workers = df.salary[df["hours-per-week"] == 1]

    rich_percentage = (num_min_workers[num_min_workers == ">50K"].count() / num_min_workers.count() * 100).round(1)

    # What country has the highest percentage of people that earn >50K?
    high_earner_percent = df["native-country"][df.salary == ">50K"].value_counts().sort_index() / df["native-country"].value_counts().sort_index()
    highest_earning_country = high_earner_percent.sort_values(ascending=False).index[0]
    highest_earning_country_percentage = (high_earner_percent.max() * 100).round(1)
